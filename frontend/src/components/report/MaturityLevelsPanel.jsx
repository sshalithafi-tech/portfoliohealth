...
export const MaturityLevelsPanel = ({ overallLevel, scores, report }) => (
  <div className="p-6 glass-surface-highlight rounded-xl">
    <h2 className="text-lg font-semibold text-[#0C1B2A] mb-6 font-['Outfit']">The Five PPDT Maturity Levels</h2>
        ...
        return (
          <div
            ...
            className={`flex-1 p-3 sm:p-4 rounded-xl border transition-all ${
              isActive ? 'bg-[#F8F9FA] shadow-lg' : 'border-[#E2E8F0] bg-[#F8F9FA]'
            }`}
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`text-xs sm:text-sm font-semibold ${isActive ? 'text-[#0C1B2A]' : 'text-[#4A5568]'}`}>{ml.name}</span>
            </div>
            <p className={`text-[11px] leading-relaxed ${isActive ? 'text-[#4A5568]' : 'text-[#8896A5]'}`}>{ml.desc}</p>
          </div>
        );
      })}
    </div>
    <div className="mt-6 space-y-2">
      {DIMENSIONS.map(d => {
        ...
        return (
          <div key={d} className="flex items-start gap-3 py-2 border-b border-[#E2E8F0] last:border-0">
            ...
            <p className="text-[#4A5568] text-sm">{interp}</p>
          </div>
        );
      })}
    </div>
