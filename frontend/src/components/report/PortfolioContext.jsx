...
const InfoTile = ({ icon: Icon, label, value, accent = "#C9A84C", testId }) => {
  ...
  return (
    <div
      data-testid={testId}
      className="flex items-start gap-3 p-3.5 rounded-xl bg-white/[0.025] border border-white/[0.05] hover:border-white/[0.10] transition-colors"
    >
      <div
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm font-semibold text-white mt-0.5 font-['Outfit'] leading-snug truncate">
          {value}
        </p>
      </div>
    </div>
...
export const PortfolioContext = ({ assessment, report }) => {
  ...
  return (
    <div data-testid="portfolio-context-card" className="glass-surface-highlight rounded-2xl overflow-hidden">
      <div className="p-5 sm:p-6 border-b border-white/[0.06] bg-gradient-to-br from-[#C9A84C]/[0.05] to-transparent">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <h2 className="text-lg sm:text-xl font-semibold text-white font-['Outfit'] leading-tight truncate">
                {companyName}
              </h2>
              {industry && (
                <p className="text-xs text-white/45 mt-0.5 truncate">{industry}</p>
              )}
            </div>
          </div>
      ...
      {report?.business_model_note && (
        <div className="px-5 sm:px-6 pb-5 -mt-1">
          <p
            ...
            className="text-[11px] italic text-white/45 leading-relaxed"
          >
            {report.business_model_note}
          </p>
        </div>
