...
export const ScoreMethodology = ({ scores, weightsNorm }) => {
  ...
  return (
    <div className="glass-surface-highlight rounded-xl overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between p-5 text-left hover:bg-[#F8F9FA] transition-colors">
        <div className="flex items-center gap-2">
          <Info size={18} className="text-[#C9A84C]" />
          <span className="text-[#0C1B2A] font-medium font-['Outfit']">How is this score calculated?</span>
        </div>
        {open ? <ChevronUp size={18} className="text-[#4A5568]" /> : <ChevronDown size={18} className="text-[#4A5568]" />}
      </button>
      {open && (
        <div className="px-5 pb-6 space-y-5 animate-fade-in border-t border-[#E2E8F0]">
          <p className="text-[#4A5568] text-sm leading-relaxed mt-4">
          ...
          <div className="p-5 bg-[#F8F9FA] rounded-xl border border-[#E2E8F0] text-center">
            ...
            <div className="text-[#4A5568] text-xs space-y-1 text-left max-w-md mx-auto">
              <p><span className="text-[#0C1B2A] font-medium italic">M</span> = Overall Maturity Score (1.0 to 5.0)</p>
              <p><span className="text-[#0C1B2A] font-medium italic">w</span> = Strategic Weight assigned to the specific pillar</p>
              <p><span className="text-[#0C1B2A] font-medium italic">S</span> = Assessed Grade (1 to 5) for the pillar</p>
          ...
          <div className="p-4 bg-[#F8F9FA] rounded-xl border border-[#E2E8F0]">
            <h4 className="text-sm font-semibold text-[#0C1B2A] mb-3 font-['Outfit']">Your Calculation</h4>
            <div className="space-y-1.5 font-['JetBrains_Mono'] text-xs">
              {DIMENSIONS.map(d => {
                const s = scores[d] || 0;
                const w = weightsNorm[d] || 0.25;
                return (
                  <div key={d} className="flex items-center gap-2 text-[#4A5568]">
                    <span className="text-[#0C1B2A] capitalize w-20">{d}</span>
                    <span>{s} x {w.toFixed(2)} = <span className="text-[#7ee787] font-semibold">{(s * w).toFixed(2)}</span></span>
                  </div>
                );
              })}
              <div className="border-t border-[#E2E8F0] pt-2 mt-2 flex items-center gap-2">
                <span className="text-[#0C1B2A] font-semibold w-20">Total</span>
