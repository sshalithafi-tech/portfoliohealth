...
from report_sections import generate_report_sections
...
After completing PHASE 4 (Confirm & Close), you MUST produce your response in TWO parts, in this exact order:

PART A — A short, warm closing message (2–4 sentences) thanking the respondent and letting them know their full PortfolioHealth report is now being generated.

PART B — A SEED JSON emission, wrapped in a single fenced code block, containing ONLY the raw facts below. Do NOT write narrative analysis, interpretations, roadmap, or a consultant's note here — those are generated by a separate specialist process immediately afterwards from this seed data plus the full conversation transcript. Populate every field using ONLY evidence gathered during this conversation.

SEED SCHEMA — wrap the JSON in a single fenced code block:
...
Scoring rules for this seed (per the SCORING SYSTEM above):
- Score each pillar 1.0–5.0 using the maturity level definitions above. Use half-point increments when evidence straddles two levels. Never assign a score without specific conversational evidence — score conservatively (round down) if ambiguous.
- `overall` = simple average of the four pillar scores, rounded to 1 decimal (this is the equal-weighted baseline; the backend recomputes the business-model-contextual score downstream — you do not need to compute it here).
- `bottleneck_pillar` = the lowest-scoring pillar. If two pillars tie for lowest, pick the one your conversation surfaced as the most limiting to portfolio decisions.

The ```json block MUST include every field shown above. Never abbreviate the block. This is the ONLY JSON you emit here — do not attempt to also emit dimension summaries, pillar interpretations, governance analysis, key findings, critical gaps, roadmap, assessment reliability, benchmark context, or consultant's note. Those sections are generated automatically immediately after this seed by three concurrent specialist calls, from this exact seed data plus the transcript — this keeps report generation fast without dropping or reordering any of the 14 report sections."""
...
async def send_chat_message(assessment_id: str, request: SendMessageRequest, current_user: dict = Depends(get_current_user)):
    ...
    seed_data = extract_report_json(response_text)
    report_data = None
    scores = None
    status = assessment.get("status", "in_progress")
    if seed_data and seed_data.get("ready_for_report"):
        try:
            report_data = await generate_report_sections(seed_data, chat_history)
        except Exception as exc:
            logging.error(f"Report section generation error: {exc}")
